...
**问题**：部署后打开页面，输入正确密码 `HYNM2026` 仍提示"密码错误或数据损坏"，无法进入甘特图。

**根因**：保存端与加载端的 base64 编码层数不对称，且初始上传文件与保存后文件格式不一致。
- 保存端 `saveToGitHub`：`cipherB64`(标准 base64 密文) → `btoa(...)` 再包一层 → 写入仓库，故仓库文件 = `btoa(cipherB64)`
- 加载端 `submitPassword`：若直接把"仓库文本"当 `cipherB64` 送 `decryptData`，等于多/少了一层 base64，AES 解密失败 → 误判为"密码错误"
- 同时用户最初上传的 `gantt-data.enc.json` 由 Python `crypto_helper.py` 生成，内容为**纯** `cipherB64`（未再 btoa），与保存后文件格式不同，混用加剧解码混乱

**修复**：
1. **统一编码格式**：明确仓库文件统一为 `btoa(cipherB64)`（即标准 base64 密文再包一层 base64），保存端与加载端对称
2. **加载端自动兼容两种格式**：先 `atob(文本)` 还原为 `cipherB64`；若解密失败则回退把原文直接当 `cipherB64` 重试，从而同时兼容 Python 生成的纯密文文件与本版保存的 btoa 包装文件
3. 用真实 `gantt-data.enc.json` + 模拟 save→load 做端到端验证：正确密码可还原 11 个任务，错误密码均抛异常（E2E PASS）

**验证**：
- 场景1（Python 格式文件）：正确密码解密成功（11 任务还原），错误密码抛异常 ✅
- 场景2（save→load 往返）：正确密码解密成功，错误密码抛异常 ✅
- 场景3（错误密码对 Python 格式）：抛异常 ✅
